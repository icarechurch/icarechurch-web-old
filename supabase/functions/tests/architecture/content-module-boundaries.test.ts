...
Deno.test("content layers preserve their dependency boundaries", async () => {
  ...
  const forbiddenInApplication =
    /@supabase\/supabase-js|\bDeno\b|\.from\(|\.rpc\(/;
  ...
  for (const file of files) {
    ...
    if (path.includes("/application/") && forbiddenInApplication.test(source)) {
      violations.push(
        `${path}: application imports runtime or persistence code`,
      );
    }
    if (
      path.includes("/presentation/") && forbiddenInPresentation.test(source)
    ) {
