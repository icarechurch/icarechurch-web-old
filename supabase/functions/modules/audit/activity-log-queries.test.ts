...
const operation = (
  handlers: ReturnType<typeof createAuditModule>["activityLogs"],
  name: string,
) =>
...
Deno.test("preserves filtered activity log query clauses", async () => {
  ...
  const handlers = createAuditModule(client as never).activityLogs;
...
Deno.test("rejects activity log pages outside the supported bounds", async () => {
  ...
  const handler = createAuditModule(client as never).activityLogs.list;
